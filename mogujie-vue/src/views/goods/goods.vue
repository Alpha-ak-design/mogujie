    ...
    <div class="good">
        ...
        <div class="swiper-container">
            <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="(item,index) in detail.s_img_more">
                    <img :src="item" />
                </div>
            </div>
            <!-- Add Pagination -->
        ...
        <div class="service">
            <div class="service-div">
                <div v-for="(item,index) in detail.s_sendDescribe">
                    <img src="https://s10.mogucdn.com/mlcdn/c45406/180417_25kbfg1c3hdbd120394ji4b11bk2k_36x36.png_80x80.v1cAC.40.webp" />
                    {{item}}
                </div>
        ...
        <div class="select" @click="goBuy">
            <div>请选择：尺码</div>
            <div class="iconfont icon-qianjin" style="font-size:12px;"></div>
        </div>
        <!--recommend-->
        <div class="reconmend">
            ...
            <div class="user" v-for="(item,index) in recommend">
                <div class="img-name">
        ...
        methods:{
            ...
            async addCart(){
                ...
                const result = await this.$http({
                    method:"post",
                    url:"index/test_ela/addCart",
                    headers:{
                        'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8'
                    },
                    data:this.$qs.stringify({goods_info:obj,user_id:this.$user_id()})
                });
                if (result.data.code == 1){
                    this.isShowBuy = false;
                    this.$alert.success(result.data.msg,"500")
                }
            },
            async collect(){
                let goods_id = this.$route.params.id;
                let user_id = this.$user_id()
                let collect = {goods_id:goods_id,user_id:user_id};
                if (this.isCollect){
                    //取消收藏
                    collect.addCollect = 0
                } else {
                    collect.addCollect = 1
                }
                const result = await this.$http({
                    method:"post",
                    url:"index/test_ela/collect",
                    headers:{
                        'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8'
                    },
                    data:this.$qs.stringify({collect})
                })
                if (result.data.code == 1){
            ...
            //是否收藏
            async IsCollect(){
                let user_id = window.localStorage.getItem("my-token");
                const result = await this.$http({
                    method:"post",
                    url:"index/test_ela/userInfo",
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8'
                    },
                    data:this.$qs.stringify({user_id:user_id})
                });
                let collect = result.data.collect
                let goods_id = this.$route.params.id
                for (let i in collect){
                    if (collect[i] == goods_id) {
                        this.isCollect = true;
                        break;
                    }
                }
